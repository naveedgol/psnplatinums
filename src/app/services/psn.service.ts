...
export class PsnService {
  ...
  constructor(private http: HttpClient) {
    this.http.get('./assets/data/images.json').subscribe(data => {
      this.dict = data;
    });
  }
  ...
  parseCount(doc, trophyClass: string): number {
    return parseInt(
      doc
        .querySelectorAll('li.' + trophyClass)[0]
        .innerText.trim()
        .replace(/,/g, '')
    );
  }
  ...
  parseUser(data: string, psn_id: string): User {
    ...
    user.level = parseInt(
      (doc.querySelectorAll('div.progress-bar.level')[0].previousSibling.previousSibling as HTMLElement).innerText
    );
    user.avatar = doc.getElementsByTagName('meta')[8].content;
    let levelProgress = doc.querySelector('div.progress-bar').childNodes[1].textContent.substring(6, 8);
    if (levelProgress[1] === '%') {
  ...
  parsePlats(data: string): Trophy[] {
    ...
    const rarities = Array.from(doc.querySelectorAll('span.typo-top')).filter(
    ...
    for (let i = 0; i < titles.length; ++i) {
      ...
      const d = (dates[i] as HTMLElement).innerText.trim().split(' ');
  ...
  async fetchProfile(psn_id: string, loadingProgress) {
    ...
    for (let i = 0; i < this.user.platinumCount; i += 50) {
      await this.getPlatinums(psn_id, i / 50 + 1)
        .toPromise()
        .then(data => {
